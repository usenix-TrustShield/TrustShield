...
import requests
import TrustShield.fl_utils.dataloader as dl
...
def load_medical_data(loc):
    train_data_path =loc+'/train'
    test_data_path = loc+'/test'
    transform = transforms.Compose(
        [
            transforms.ToTensor(),
         transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
...
def load_nlp_data(file, new=False):
    """
...
def split_and_shuffle_labels_nlp(y_data, seed, amount, n):
    """
    A function to take all labels and assign a new position to that one among other positions of the same label
    y_data : the labels of the set
    amount : the number of sample used in the process
...
def distribute_mnist_data_to_participants(label_dict, amount, number_of_samples, n,
    ...
    for i in range(number_of_samples):
        ...
        for j in range(n):
            ...
            # print(amount_info_table)
            if label != -1:
                label_amount = amount_info_table.iloc[i, j]
                index_counter.loc[label, "end"] = index_counter.loc[label, "end"] + label_amount
                node_data_indices = pd.concat([node_data_indices, label_dict_data.loc[index_counter.loc[label, "start"]:index_counter.loc[label, "end"] - 1,"i"]])
                index_counter.loc[label, "start"] = index_counter.loc[label, "end"]
        # print("Shape of node_data_indices:", node_data_indices.shape)
        # print("Index values before reset_index:", node_data_indices.iloc[:, 0])
        # print("Index values after reset_index:", node_data_indices.iloc[:, 0].reset_index(drop=True))
        # print(i)
        # print("success3")
        x_info = x_data[node_data_indices.iloc[:, 0].reset_index(drop=True), :]
        # print("success4")
...
def distribute_nlp_data_to_participants(label_dict, amount, number_of_samples, n,
