...
def basic_plot_ts(x,y, ltype, legendd, legloc, mtype, title_str = None, plot_file = None, ylabel = None, lw=3.0, ylim = 'none', xlim = 'none', xlabel = 'time', alp=1.0, msize=10):
    sns.color_palette("rocket", as_cmap=True)
    for i in range(len(x)):
        plt.plot(x[i], y[i], ltype[i], color=ltype[i][-1],lw=lw,mfc=mtype[i], markersize=msize, alpha=alp)
    ...
    if len(legendd):
        plt.legend(legendd, bbox_to_anchor=legloc, ncol=len(legendd))
    else:
        plt.legend(legendd, bbox_to_anchor=legloc)
    plt.title(title_str)
    plt.savefig(plot_file)
    plt.close()
