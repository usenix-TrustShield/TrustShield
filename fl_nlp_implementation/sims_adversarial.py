...
def run_sim(opt, device):
    ...
    if apply_mining:
        label_dict_validators = \
            dd.split_and_shuffle_labels(y_data=y_test, seed=seed, amount=test_amount, n=n)
        node_label_info_validators, _, amount_info_table_validators = \
            dd.get_info_for_validator_nodes(number_of_validators=number_of_validators,
                                            n=validator_num_classes,
                                            amount=test_amount,
                                            seed=use_seed,
                                            distribution=validator_dist)
        x_val_dict, mask_val_dict, y_val_dict = \
            dd.distribute_data_to_participants(label_dict=label_dict_validators,
                                               amount=test_amount,
                                               number_of_samples=number_of_validators,
                                               n=validator_num_classes,
                                               x_data=x_test,
                                               mask_data=mask_test,
                                               y_data=y_test,
                                               node_label_info=node_label_info_validators,
                                               amount_info_table=amount_info_table_validators,
                                               x_name="x_val",
                                               mask_name="mask_val",
                                               y_name="y_val",
                                               randomization_flag=False)
    ...
    for iteration in tqdm(range(iteration_num)): # communication rounds
        ...
        model_dict = \
            tn.send_main_model_to_nodes_and_update_model_dict(main_model,
                                                              model_dict,
        ...
        if apply_mining and initialization_rounds <= iteration:
            if is_noniid:
                if unbias_mechanism: # unbias the models and change the edge models dictionary accordingly
                    for client in range(number_of_samples):
                        model_dict["model" + str(client)] = \
                            tn.unbias_noniid_edge_update(main_model,
                                                         model_dict["model" + str(client)],
                                                         device)
                else:
                    print("WARNING: There is no unbiasing mechanism used!")
            mining_results_loss, mining_results_acc, miner_names, validated_edges, blacklisted_edges = \
                tn.start_mining_validator_node_cifar(number_of_samples,
                                                     ...
                                                     batch_size,
                                                     iteration,
                                                     validation_threshold=max(test_accuracy/warranty_factor, initial_cut),
            ...
            print(mining_history["validated_edges"])
            print(len(validated_edges.tolist())/number_of_samples)
    ...
    np.save("./histories/NLP/FLNLP_mining_{}_noniidedge_{}_{}_ratio_{}.npy".format( apply_mining, is_noniid, validator_dist, hostile_node_percentage), test_accuracies_of_each_iteration)
...
if __name__ == "__main__":
    ...
    adv = [0.5]
    device1 = torch.device("cuda:"+str(4) if (torch.cuda.is_available()) else "cpu")
    device2 = torch.device("cuda:"+str(5) if (torch.cuda.is_available()) else "cpu")
    device3 = torch.device("cuda:"+str(6) if (torch.cuda.is_available()) else "cpu")
    device4 = torch.device("cuda:"+str(1) if (torch.cuda.is_available()) else "cpu")
