...
def validation(model, test_loader, criterion, device, set_evaluation_flag):
...
def start_train_end_node_process_cifar(number_of_samples, x_train_dict, y_train_dict, x_test_dict, y_test_dict, batch_size, model_dict, criterion_dict, optimizer_dict, numEpoch, device,clipping=False, clipping_threshold =10):
    ...
    # print("debug",x_train_dict[name_of_x_train_sets[0]])
    ...
    for i in range(number_of_samples): # TODO this is not in parallel, multiprocessing can be implemented here for speed purposes
        #print(i,x_train_dict[name_of_x_train_sets[i]])
        ...
        for _ in range(numEpoch):
            train_loss, train_accuracy = train_with_augmentation(model, train_dl, criterion, optimizer, device,
                                                                 ...
                                                                 use_augmentation=False, augment=transform_augment)
            #print("train_loss,train_accuracy",train_loss,train_accuracy)

            test_loss, test_accuracy = validation(model, test_dl, criterion, device,set_evaluation_flag=False)
            #print("test_loss,test_accuracy",test_loss,test_accuracy)
    
        # TODO validation can also be implemented here for different dataset distributions by just changing the dataset distribution of the validation devices
        # TODO however, the validation devices should also have seperate dictionaries for themselves including models minimizers and devices
        # TODO these metrics can be saved as histogram of the edge devices into the seperate folder of the edge device
...
def set_averaged_weights_as_main_model_weights_and_update_main_model(main_model, model_dict, device):
    ...
    main_model_param_data_list = list(main_model.parameters())
    #print('set average',main_model_param_data_list)
    with torch.no_grad():
        for j in range(len(main_model_param_data_list)):
            main_model_param_data_list[j].data = mean_weight_array[j].detach().clone()
    for j in range(len(main_model_param_data_list)):    
       assert (mean_weight_array[j] == list(main_model.parameters())[j].data).all(), "Assertion failed for main model update"
    # print('final main model', list(main_model.parameters())[0].data[0])
    # print("final mean array",)
    return main_model
def start_mining_validator_node_cifar(number_of_edge_clients, number_of_validators, x_val_dict, y_val_dict, model_dict, criterion_dict, device, batch_size, current_round,validation_threshold=0.2, initialization_rounds = 20, config="one_to_all", rgRatio=0.3):
    ...
    if config=="one_to_all":
        ...
        for i in range(number_of_validators): # TODO this is not in parallel, multiprocessing can be implemented here for speed purposes
            ...
            for j in range(number_of_edge_clients):
                ...
                val_loss, val_accuracy = validation(unverified_edge_model, val_dl, unverified_edge_criterion, device,set_evaluation_flag=False)
    ...
    print("val threshold",validation_threshold)
    print("validated edges",validated_edges)
    print('blacklisted edges',blacklisted_edges)
