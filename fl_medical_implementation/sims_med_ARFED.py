...
def run_sim(opt, device):
    ...
    # print("step4 done",x_train_dict,y_train_dict)
    if apply_mining:
    ...
    test_accuracy = 0
    initialization_rounds = 3
    its = np.array([], dtype=int)
    distances = pd.DataFrame()
    thresholds = pd.DataFrame()

    for iteration in range(iteration_num): # communication rounds
    ...
    # with open("./histories/hist_FLCIFAR10_mining_"+str(apply_mining)+"_noniidedge_"+str(is_noniid)+"_"+str(validator_dist)+"_organized_numclients_100_ratio_"+str(hostile_node_percentage)+".txt", "w") as fp:
    #     json.dump(mining_history, fp)  # encode dict into JSON
    print("Simulation is complete!")
    #return test_accuracies_of_each_iteration[-1]
    np.save("./histories/FLMEDICAL/FLMEDICAL_ARFED_isnoniid_"+str(opt.isnoniid)+"_"+str(hostile_node_percentage)+".npy", test_accuracies_of_each_iteration)
